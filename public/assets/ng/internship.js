...
app.controller('InternshipOverseasCtrl', [
  ...
  function($scope, restService) {
        ...
        restService.getAvailableCountry().then(function(countries){
            ...
            for(var i = 0 ; i < ava_countries.length ; ++i) {
              if(ava_countries[i].name == "Thailand") {
                ...
                $scope.filter.push(ava_countries[i].name)
              }
            }
            if(thai != -1){
    ...
    $scope.filterplace = function(place) {
      ...
      if($scope.filter.indexOf(place.country_name) == -1) {
        return false
      }
      return true
    }
    ...
    $scope.filtercountry = function(val, option) {
      var index = $scope.filter.indexOf(option.name)
      if(val) {
        if(index == -1) {
          $scope.filter.push(option.name)
        }
      } else {
        if(index != -1) {
          $scope.filter.splice(index,1)
